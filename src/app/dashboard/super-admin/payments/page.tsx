    ...
    <div className="flex flex-col gap-6 w-full max-w-6xl">
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {summaryCards.map((card) => {
          ...
          return (
            <div
              key={card.label}
              className="rounded-xl border border-slate-200 dark:border-slate-800 bg-surface dark:bg-slate-900 p-4 flex flex-col gap-3"
            >
      ...
      <section className="rounded-xl border border-slate-200 dark:border-slate-800 bg-surface dark:bg-slate-900 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-800">
          ...
          <button
            type="button"
            className="inline-flex items-center justify-center rounded-full border border-slate-200 dark:border-slate-700 px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"
          >
            View full history
          </button>
        </div>

        <div className="grid grid-cols-12 gap-4 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">
        ...
        <div className="divide-y divide-slate-200 dark:divide-slate-800">
          {payments.map((txn) => (
            ...
            >
              <div className="col-span-3 font-mono text-xs text-slate-500 dark:text-slate-400">{txn.id}</div>
              <div className="col-span-3 text-slate-900 dark:text-slate-50">{txn.name}</div>
              <div className="col-span-2 text-slate-600 dark:text-slate-300">{txn.method}</div>
              <div className="col-span-2 font-semibold text-slate-900 dark:text-slate-50">{txn.amount}</div>
